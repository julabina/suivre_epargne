<template>
    ...
    <AuthenticatedLayout>
        ...
        <form @submit.prevent="onSubmit">
            ...
            <div class="">
                <input type="submit" value="Modifier" class="p-4 bg-blue-500">
            </div>
        </form>
    </AuthenticatedLayout>
</template>
...
<script setup>
    ...
    onMounted(() => {
        ...
        for (const [key, value] of Object.entries(loca)) {
            locaValue.push(value);
            locaKey.push(key);
        }

        form.title = props.project.title;
        form.description = props.project.description;
        form.amountGoal = props.project.goal_amount;
        
        if (locaValue.includes(props.project.location)) {            
            for (let i = 0; i < locaValue.length; i++) {
                if (props.project.location === i) {
                    form.location = props.project.location;
                }
            }
        } else {
            form.customLocation = props.project.location;
            form.toggleCustomLocation = true;
        }

        if (props.project.deadline) {  
            const today = new Date();
            today.setDate(today.getDate() + 1); 
            let customDate = new Date(props.project.deadline);
            
            if (customDate > today) {   
                form.deadline = customDate.toISOString().split('T')[0];
                form.toggleDeadline = true;
            }
        }
    });        
    ...
    const onSubmit = async () => {
        ...
        if (result) {
            console.log(form.customLocation);
            
            return router.visit(route('project.update', { id: props.project.id }), {
                method: 'put',
                data: {
                    form: form
                }
